...
def price_history_data_to_gcp(**kwargs):
    ...
    # Note: First blob from source_bucket.list_blobs is folder PriceHistory/, it isn't file.
    # For this we need to exclude first item from list with slise.
    blobs_names = list(
        [blob.name for blob in source_bucket.list_blobs(prefix='PriceHistory/')]
    )[1::]
    blobs_names.sort(key=lambda x: int(re.search(r'\d+', x).group(0)))

    for blobs_names_chunk in chunk(blobs_names, MAX_FILES_PER_CHUNK):
        destination_blob_name = f"PriceHistory/" \
...
with DAG(
        dag_id='price_history_data_to_gcp',
    ...
    PythonOperator(
        task_id='price_history_data_to_gcp',
        python_callable=price_history_data_to_gcp,
        dag=dag,
